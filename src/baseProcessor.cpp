...
namespace lsp_proxy {
  ...
  bool BaseProcessor::readPipe()
  {
    ...
    while (true)
    {
      ...
      else // Found terminator
      {
        ...
        if (typePos != -1)
          std::cout << "FOUND CONTENT TYPE HEADER\n";
        ...
        if (lengthPos == -1 && lengthPos == -1) // End of header
        {
          ...
          {
            auto const content = m_message.substr(headerPos, contentLength);
            writePipe(process(content));
  ...
  std::string BaseProcessor::process(std::string const & message)
  {
    return message;
  }
